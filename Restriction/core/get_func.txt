...
async def get_msg(userbot, sender, edit_id, msg_link, edit):
    ...
    if 't.me/c/' in msg_link or 't.me/b/' in msg_link or 't.me/' in msg_link:
        ...
        try:
            ...
            media_files = [msg]  # List of media to download

            try:
                files = await parallel_download_media(userbot, media_files, edit)
            except Exception as e:
            ...
            for file, media in zip(files, media_files):
                thumb_path, caption = await thumb_caption(userbot, sender, media, file)

                if media.media == MessageMediaType.VIDEO:
                ...
                elif media.media == MessageMediaType.PHOTO:
